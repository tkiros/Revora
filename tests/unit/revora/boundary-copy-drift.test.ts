...
import { buildOutOfScopeResponse } from "../../../lib/revora/fallback";
 ...
 *
 * `lib/revora/boundary-copy.ts` holds the client-safe literals every surface
...
describe("every consuming surface renders the module strings", () => {
  ...
  it("onboarding and profile route import boundary copy and carry no inline literal", () => {
    for (const rel of [
      ...
      expect(
        src,
        `${rel} must import from lib/revora/boundary-copy`
      ).toMatch(/from\s+["'].*revora\/boundary-copy["']/);
      expect(
        src,
