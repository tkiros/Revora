 ...
 * check engine (lib/revora/*) must not import any meal-memory module, so a
 * memory physically cannot reach the card-band decision, no matter what a future
 * edit does inside the engine.
 *
 * This is the copy-pins source-scan pattern (tests/unit/revora/copy-pins,
...
describe("meal memory never feeds the check engine", () => {
  const engineFiles = collectTsFiles("lib/revora");
  ...
  it.each(FORBIDDEN_IMPORT_PATTERNS.map((p) => [p.source, p] as const))(
    "no lib/revora source references %s",
    (_label, pattern) => {
