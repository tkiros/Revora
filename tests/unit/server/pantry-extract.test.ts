...
describe("pantry vision extraction", () => {
  ...
  it("parses items and sends in-memory bytes instead of the private Blob URL", async () => {
    ...
    const loadPhoto = vi
      .fn()
      .mockResolvedValue("data:image/jpeg;base64,cGhvdG8=");
    ...
    expect(loadPhoto).toHaveBeenCalledWith("https://blob.test/p1.jpg");
  ...
  it("throws on non-JSON output (caller marks the photo failed)", async () => {
    const client = createPantryVisionClient({
      ...
      loadPhoto: vi.fn().mockResolvedValue("data:image/jpeg;base64,eA==")
    });
