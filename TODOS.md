...
## support@revora.plus BOUNCES — the refund door notifies nobody (P0, owner)
- **What:** the C7 support-case email path is broken at the last hop. Test case `3a623ed1` (2026-07-22) returned `emailed:true` and rendered its case id, but Resend shows the mail to `support@revora.plus` bounced ("Generic Temporary Delivery Failure"). `revora.plus` MX is Namecheap email forwarding (`eforward*.registrar-servers.com`); no working forwarding rule for `support@` means every case email — and the pantry-sweep cron's reports (`lib/server/pantry/sweep.ts:188`) — bounces. Case rows are safe in `support_cases` (row-first design), but the published "we reply within 2 business days" SLA can't be met because nobody sees the cases.
- **Fix (owner, ~2 min):** Namecheap dashboard → revora.plus → Redirect Email → add rule `support` → monitored inbox (e.g. terrykiros@gmail.com). Then send any email to support@revora.plus to confirm forwarding, and re-run an in-app case to confirm the full path. If a rule already exists, the temp-failure is on Namecheap's side (greylisting) — consider a real mailbox or Resend Receiving instead.
- **Why it matters:** this is the refund door. A user whose help request silently disappears files a chargeback instead. `SUPPORT_EMAIL` is hardcoded (`lib/revora/contact.ts:18`) and printed in the privacy policy and pantry emails, so keep the address and fix delivery rather than changing the address.
