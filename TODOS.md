...
## `next-env.d.ts` goes dirty after any `next dev` — teardown only heals the trial marker
- **What:** the tracked `next-env.d.ts` imports `./.next/types/routes.d.ts`, but Next 16's dev server rewrites that line to `./.next/dev/types/routes.d.ts` on boot. `tests/smoke/global-teardown.ts` only restores the `.next/e2e-trial` marker (the :3101 server's distDir), so the DEFAULT `:3100` server's `.next/dev` rewrite matches nothing and survives the teardown. Observed repeatedly on 2026-07-22 — every Playwright run, and plain `npm run dev` too, left the file modified and it had to be restored by hand.
- **Why it matters:** the teardown exists precisely so a smoke run leaves a clean tree (its own docstring: "a clean-tree gate or a tsc step would fail, or the e2e-trial paths could be committed by accident"). That guarantee is half-true today, and the wrong path can be committed by accident from any dev session.
- **Fix is a decision, not a patch:** `next dev` and `next build` write DIFFERENT values into this file, so "just commit what dev writes" trades one dirty state for the other. Options: heal `.next/dev` in the teardown too (fixes e2e only, not `npm run dev`), gitignore the file and generate it (Next supports this; it is a generated artifact), or pin `typedRoutes` config so both commands agree. Pick one.
