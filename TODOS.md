...
## Umami analytics is CSP-blocked in production — the activation funnel has NO data
- **What:** `next.config.ts` derives the umami CSP token from `NEXT_PUBLIC_UMAMI_SRC`, which is the *script* host (`https://cloud.umami.is`). The tracker it loads POSTs its events to a DIFFERENT host, `https://gateway.umami.is/api/send`, which `connect-src` does not allow. Live console on revora.plus/home 2026-07-22: `Connecting to 'https://gateway.umami.is/api/send' violates the following Content Security Policy directive: "connect-src …"` + `Fetch API cannot load … Refused to connect`. Every `track()` call is silently dropped.
- **Why it matters:** this is exactly the failure mode PR #25 just fixed for Sentry (script origin allowed, ingest origin not), and it kills the numbers the funnel is judged on — `onboarding_started` is described in `app/(app)/onboarding/page.tsx` as the denominator without which onboarding drop-off "was not computable at all". `attribution`, `onboarding_completed` and the rest are equally gone. Any funnel figure quoted from umami to date should be treated as missing, not zero.
- **Fix:** add the ingest origin to `connect-src` alongside the script origin (same `originFromEnv` helper), e.g. a second env var or an explicit `https://gateway.umami.is`. Confirm the real endpoint for the account first — umami cloud has changed ingest hosts before — then re-check the live console for zero CSP violations.
- **Pre-existing:** NOT introduced by PR #25. The https-only `originFromEnv` rewrite is behaviour-identical for the https umami URL; this was broken before and after. Found while running the post-deploy verification.
