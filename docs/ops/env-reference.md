...
| `NEXT_PUBLIC_SENTRY_DSN` | P0.3 | Client-side error reporting (`components/client-error-reporting.tsx`); messages/breadcrumbs/query strings are stripped before send. Without it, browser errors are invisible |
| `REVORA_ALLOW_NO_MEASUREMENT` | P0.3 | Exact `1` waives the production build gate that otherwise fails when the Umami/Sentry client vars are unset (`next.config.ts`). Use only for a deliberate dark launch |
...
| ~~`SUPPORT_EMAIL`~~ | **retired 2026-07-21** | Now the constant `SUPPORT_EMAIL` in `lib/revora/contact.ts`. It was never settable on `/progress` (a `"use client"` page cannot read a non-`NEXT_PUBLIC_` var), so that surface hardcoded a literal and drifted; the address is public and identical in every environment, so the variable bought nothing and hid a hole. `tests/unit/revora/owned-domains.test.ts` now fails the build if any address lands on a domain we do not own |
| ⚙ `AUTH_EMAIL_FROM` | P5 | envelope sender override, e.g. `Revora <signin@send.revora.plus>` for preview. Defaults to `Revora <signin@contact.revora.plus>` (`lib/revora/contact.ts`) — `contact.revora.plus` is the domain verified in Resend (2026-07-21). The sending domain must be verified in Resend with DKIM published, or every magic link silently fails SPF/DKIM alignment and lands in spam — which on a passwordless product is a login outage, not a deliverability nit |
