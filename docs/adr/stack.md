...
|---|---|---|
| Database | **Neon Postgres** (Vercel Marketplace) + **Drizzle ORM/kit** migrations | Serverless-native Postgres with branching (dev/preview/prod); Drizzle is schema-as-TS with plain SQL migrations, no runtime magic |
...
| Analytics | **Plausible** script + a typed no-PII event allowlist (`lib/client/analytics.ts`) | Cookieless, no consent banner needed for US launch; the allowlist is enforced by a unit test |
