...
**⚠ REMAINING PROD BLOCKER — root-caused: OpenAI account is on a minimal/trial tier (no real billing).** With the middleware fixed, `POST /api/check` reaches the model but returns the calm `retry` fallback on every real model call. Diagnosis (owner supplied a fresh key, validated end-to-end):

- The supplied key is valid and `gpt-5.4-mini` is accessible; the app's exact code path (Responses API + strict JSON schema) succeeds **8/8 locally** in 1.6–3.7s. Prod key confirmed replaced (single `OPENAI_API_KEY` entry, redeploy `revora-8gh4pjupr`).
- **But the key is Tier-0/trial:** `x-ratelimit-limit-requests: 50`, `x-ratelimit-limit-tokens: 100000`, ~6h reset — and general billing is absent (`gpt-4o-mini` → `insufficient_quota`; `gpt-5.4-mini` served only from a tiny trial allowance).
- In prod this surfaces two ways in the telemetry (`latencyBucket` `<2s` and `5-12s`): fast **429 rate-limits** (the 50-request cap trips under any traffic — `remaining` fell 37→35 on two test pings) and **slow, deprioritized reasoning calls** that exceed the 10s client SDK timeout. Local few/warm/spaced calls dodge both.

**Resolution is an OpenAI-account action only the owner can take: add a payment method + credits at platform.openai.com → Settings → Billing (and raise the usage tier).** That removes the quota wall, lifts the 50-req cap, and restores normal inference priority/latency. No key/model/code change fixes an under-provisioned account. Secondary (only if latency is still tight post-billing): raise the 10s client timeout in `lib/revora/openai-client.ts` and `maxDuration`, mindful of the ~12s browser abort. The live graded eval (BUG-09) should be run once the account is provisioned, before the `PAYWALL_MODE=trial` flip.
